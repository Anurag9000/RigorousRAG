...
This module removes the manual handoff between training/export qualification and serving-stack
promotion. It never loads model weights: persisted artifact bytes are re-verified, promotion
evidence must be self-valid and promoted, and the resulting runtime component contract binds
the exact artifact/runtime lineage. Aggregate offline-quality evidence is then bound to the
exact runtime stack that contains those components.
"""
...
from orchestration.runtime_stack_authority import RuntimeComponent, RuntimePromotionEvidence, RuntimeStackArtifact
...
def _component_contract(manifest: AdvancedArtifactManifest) -> str:
    return _digest(
        {
            "schema": "rigorousrag-advanced-runtime-component-contract/v1",
...
class AdvancedRuntimeComponentBinding:
    ...
    def __post_init__(self) -> None:
        ...
        if self.artifact_kind not in {"grounded_generator", "dynamic_rag_policy"}:
            raise ValueError("unsupported advanced artifact kind")
        for name in ("artifact_sha256", "promotion_evidence_sha256", "evaluation_receipt_sha256", "binding_sha256"):
    ...
    def _payload(self) -> dict[str, Any]:
        return {
            "schema": "rigorousrag-advanced-runtime-component-binding/v1",
...
def bind_qualified_advanced_artifact(
    ...
    *,
    ...
    """Create one runtime component only from exact bytes plus promoted qualification evidence."""
    if not isinstance(manifest, AdvancedArtifactManifest):
        raise ValueError("manifest must be AdvancedArtifactManifest")
    if not isinstance(promotion, AdvancedPromotionEvidence):
    ...
    if manifest.evaluation_receipt_sha256 is not None and manifest.evaluation_receipt_sha256 != promotion.evaluation_receipt_sha256:
...
def build_runtime_stack_with_advanced_bindings(
    *,
    ...
    if not bindings or any(not isinstance(value, AdvancedRuntimeComponentBinding) for value in bindings):
        raise ValueError("advanced_bindings must be a non-empty AdvancedRuntimeComponentBinding sequence")
...
def advanced_offline_quality_evidence(
    ...
    *,
    ...
    """Translate all advanced qualifications in a stack into one stack-bound offline-quality row."""
    if not isinstance(stack, RuntimeStackArtifact):
        raise ValueError("stack must be RuntimeStackArtifact")
    selected = tuple(sorted(bindings, key=lambda value: (value.component.kind, value.component.component_id)))
    if not selected or any(not isinstance(value, AdvancedRuntimeComponentBinding) for value in selected):
        raise ValueError("bindings must be a non-empty AdvancedRuntimeComponentBinding sequence")
    stack_components = {(value.kind, value.component_id, value.artifact_sha256, value.contract_sha256) for value in stack.components}
    for binding in selected:
        component = binding.component
        if (component.kind, component.component_id, component.artifact_sha256, component.contract_sha256) not in stack_components:
            raise ValueError("advanced qualification binding is not an exact component of the runtime stack")
    evidence_sha = _digest(
        {
            "schema": "rigorousrag-advanced-runtime-offline-quality/v1",
