...
def _travel_offsets(
    ...
    # For each selected node, derive the minimum cumulative travel time along an allowed
    # directed path. For upstream queries, paths are evaluated node -> anchor and the same
    # positive lag magnitude is used to shift the evidence window earlier.
    for node in node_ids:
        ...
        for path in paths:
            ...
            if valid:
                totals.append(total)
        if totals:
            offsets[node] = min(totals)
        else:
            offsets[node] = 0.0
    unresolved = tuple(f"travel_time_missing:{reach_id}" for reach_id in sorted(missing))
...
def plan_hydrology_retrieval(
    ...
    *,
    ...
    node_ids, reach_ids, unresolved = _scope_members(network, spec)
    ...
    if spec.start_time is not None and spec.end_time is not None:
        if windows:
            temporal = TimeEnvelope(min(item.start_time for item in windows), max(item.end_time for item in windows))
        else:
            temporal = TimeEnvelope(spec.start_time, spec.end_time)
    ...
    # SpatiotemporalQuery requires at least one constraint. Topology-only retrieval is
    # handled by scanning a bounded copy of index records through a neutral modality set.
    if spec.spatial is None and temporal is None and not spec.variable and not spec.modalities:
        candidates = tuple(index._records.values())[:limit]  # bounded reference-index introspection
    else:
