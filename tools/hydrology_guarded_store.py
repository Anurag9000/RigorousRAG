...
def _topology_sources(network: Any) -> tuple[str, ...]:
    return _sources(
        [item.source_id for item in network.nodes.values()]
        + [item.source_id for item in network.reaches.values()]
    )


def _package_sources(package: Any) -> tuple[str, ...]:
    return _sources(
        [item.source_id for item in package.records]
        + [item.source_id for item in package.objects]
    )
...
class GuardedHydrologyArtifactStore(HydrologyArtifactStore):
    ...
    def _preflight(self, envelope: HydrologyArtifactEnvelope, artifact: Any) -> None:
        ...
        if kind == "engineering_package":
            require_fresh(
                ...
                owner,
                kind="topology",
                fingerprint=artifact.topology_fingerprint,
            )
        ...
        if kind == "retrieval_plan":
            ...
            if artifact.package_fingerprint:
                require_fresh(
                    ...
                    owner,
                    kind="package",
                    fingerprint=artifact.package_fingerprint,
                )
            return
        if kind == "evidence_projection":
            ...
            require_fresh(
                ...
                owner,
                kind="plan",
                fingerprint=artifact.plan_fingerprint,
            )
        ...
        if kind == "evidence_report":
            ...
            require_fresh(
                ...
                owner,
                kind="projection",
                fingerprint=artifact.projection_fingerprint,
            )
            require_sources_active(self._ledger, owner, _row_sources(artifact))
            return
    ...
    def _register(self, owner: str, upstream: DependencyRef, downstream: DependencyRef, relation: str) -> None:
        self._ledger.register_dependency(
            owner,
            upstream=upstream,
    ...
    def _register_lineage(self, envelope: HydrologyArtifactEnvelope, artifact: Any) -> None:
        ...
        if envelope.kind == "topology":
            for source_id in _topology_sources(artifact):
                self._register(
                    owner,
                    DependencyRef("source", source_id),
                    downstream,
                    "source_hydrology_topology",
                )
            return
        if envelope.kind == "engineering_package":
            self._register(
                owner,
                _dep("topology", artifact.topology_fingerprint),
                downstream,
                "hydrology_topology_package",
            )
        ...
        if envelope.kind == "retrieval_plan":
            self._register(
                ...
                _dep("topology", artifact.topology_fingerprint),
                downstream,
                "hydrology_topology_plan",
            )
            if artifact.package_fingerprint:
                self._register(
                    owner,
                    _dep("package", artifact.package_fingerprint),
                    downstream,
                    "hydrology_package_plan",
                )
            return
        if envelope.kind == "evidence_projection":
            self._register(
                ...
                _dep("topology", artifact.topology_fingerprint),
                downstream,
                "hydrology_topology_projection",
            )
            self._register(
                owner,
                _dep("package", artifact.package_fingerprint),
                downstream,
                "hydrology_package_projection",
            )
            self._register(
                owner,
                _dep("plan", artifact.plan_fingerprint),
                downstream,
                "hydrology_plan_projection",
            )
        ...
        if envelope.kind == "evidence_report":
            self._register(
                ...
                _dep("topology", artifact.topology_fingerprint),
                downstream,
                "hydrology_topology_report",
            )
            self._register(
                owner,
                _dep("package", artifact.package_fingerprint),
                downstream,
                "hydrology_package_report",
            )
            self._register(
                owner,
                _dep("plan", artifact.plan_fingerprint),
                downstream,
                "hydrology_plan_report",
            )
            self._register(
                owner,
                _dep("projection", artifact.projection_fingerprint),
                downstream,
                "hydrology_projection_report",
            )
            for source_id in _row_sources(artifact):
                self._register(
                    owner,
