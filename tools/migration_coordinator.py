...
class MigrationCoordinator:
    def __init__(self, path: str | Path, *, leases: LeaseCoordinator | None = None) -> None:
        ...
        self.leases = leases or LeaseCoordinator(
            self.path.with_name(self.path.stem + "_leases.sqlite3")
        )
        with self._connect() as db:
            db.executescript(
                ...
                CREATE TABLE IF NOT EXISTS migration_step (
                  ...
                  FOREIGN KEY(owner_id, tx_id) REFERENCES migration_tx(owner_id, tx_id)
                    ON DELETE CASCADE);
                """
            )
    ...
    def _record(row: sqlite3.Row) -> MigrationRecord:
        return MigrationRecord(
            ...
            str(row["resource_id"]),
            str(row["state"]),
            tuple(json.loads(str(row["names_json"]))),
    ...
    def _parts(parts: Sequence[MigrationParticipant]) -> tuple[MigrationParticipant, ...]:
        ...
        for item in selected:
            if not all(
                callable(getattr(item, method, None))
                for method in ("prepare", "commit", "rollback")
            ):
    ...
    def begin(
        ...
        *,
        ...
        transaction_id: str,
        resource_id: str,
        participants: Sequence[MigrationParticipant],
        payload: Mapping[str, Any],
    ) -> MigrationRecord:
        owner = normalize_owner_id(owner_id)
        tx = _id(transaction_id, "transaction_id")
        resource = _id(resource_id, "resource_id")
        ...
        try:
            ...
            row = db.execute(
                "SELECT * FROM migration_tx WHERE owner_id=? AND tx_id=?",
                (owner, tx),
            ).fetchone()
            if row is not None:
                ...
                if (
                    ...
                    or record.participant_names != names
                    or dict(record.payload) != dict(payload)
                ):
                    raise ValueError(
                        "transaction_id is already bound to different inputs."
                    )
                db.execute("COMMIT")
                return record
            db.execute(
                "INSERT INTO migration_tx VALUES(?,?,?,'created',?,?,0,?,NULL)",
                (owner, tx, resource, names_json, payload_json, now),
            )
            for name in names:
                db.execute(
                    "INSERT INTO migration_step VALUES(?,?,?,0,0,0,?)",
                    (owner, tx, name, now),
                )
            row = db.execute(
                "SELECT * FROM migration_tx WHERE owner_id=? AND tx_id=?",
    ...
    def _state(
        ...
        with self._connect() as db:
            db.execute(
                "UPDATE migration_tx SET state=?,fencing_token=?,updated_at=?,last_error=? "
                "WHERE owner_id=? AND tx_id=?",
                (
                    state,
                    token,
                    time.time(),
    ...
    def _steps(self, record: MigrationRecord) -> dict[str, sqlite3.Row]:
        with self._connect() as db:
            rows = db.execute(
                "SELECT * FROM migration_step WHERE owner_id=? AND tx_id=?",
                (record.owner_id, record.transaction_id),
            ).fetchall()
    ...
    def _done(self, record: MigrationRecord, name: str, column: str) -> None:
        ...
        with self._connect() as db:
            db.execute(
                f"UPDATE migration_step SET {column}=1,updated_at=? "
                "WHERE owner_id=? AND tx_id=? AND name=?",
    ...
    def _rollback(
        self,
        record: MigrationRecord,
        parts: tuple[MigrationParticipant, ...],
        lease: LeaseRecord,
        ...
        self._state(record, "rolling_back", lease.fencing_token, error_text)
        errors: list[str] = []
        ...
        final_error = "; ".join(errors) if errors else f"compensated after {error_text}"
        self._state(
            record,
            "manual_intervention" if errors else "rolled_back",
            lease.fencing_token,
            final_error,
        )
        result = self.get(
    ...
    def execute(
        ...
        *,
        ...
        if record is None:
            raise KeyError("unknown migration transaction.")
        parts = self._parts(participants)
        ...
        lease = self.leases.acquire(
            owner_id=record.owner_id,
            resource_id=f"migration:{record.resource_id}",
            holder_id=_id(coordinator_id, "coordinator_id"),
            ttl_seconds=lease_ttl_seconds,
        )
        if lease is None:
            raise RuntimeError("migration resource is leased by another coordinator.")
        try:
            if record.state == "rolling_back":
                return self._rollback(
                    ...
                    parts,
                    lease,
                    record.last_error or "recovered interrupted rollback",
                )
            try:
                self._state(record, "preparing", lease.fencing_token)
                for item in parts:
                    if bool(self._steps(record)[item.name]["prepared"]):
                        continue
                    if not self.leases.validate_fence(lease):
                        raise RuntimeError(
                            "coordinator lease expired during prepare."
                        )
                    item.prepare(
                        transaction_id=record.transaction_id,
                        fencing_token=lease.fencing_token,
                ...
                self._state(record, "prepared", lease.fencing_token)
                self._state(record, "committing", lease.fencing_token)
                for item in parts:
                    if bool(self._steps(record)[item.name]["committed"]):
                        continue
                    if not self.leases.validate_fence(lease):
                        raise RuntimeError(
                            "coordinator lease expired during commit."
                        )
                    item.commit(
                        transaction_id=record.transaction_id,
                        fencing_token=lease.fencing_token,
    ...
    def recover(
        self,
        *,
        participants_by_name: Mapping[str, MigrationParticipant],
        coordinator_id: str,
        ...
        with self._connect() as db:
            if owner is None:
                rows = db.execute(
                    "SELECT * FROM migration_tx WHERE state NOT IN "
                    "('committed','rolled_back','manual_intervention') "
                    "ORDER BY updated_at LIMIT ?",
                    (limit,),
                ).fetchall()
            else:
                rows = db.execute(
                    "SELECT * FROM migration_tx WHERE owner_id=? AND state NOT IN "
                    "('committed','rolled_back','manual_intervention') "
                    "ORDER BY updated_at LIMIT ?",
                    (owner, limit),
                ).fetchall()
        ...
        for row in rows:
            ...
            try:
                parts = tuple(
                    participants_by_name[name] for name in record.participant_names
                )
            except KeyError:
                continue
            recovered.append(
