...
def _result_payload(result: StoredResearchResult) -> Mapping[str, Any]:
    return {
    ...
    *,
    ...
    async def run_research_query(
        ...
        if (request.session_id is None) != (request.expected_session_fingerprint is None):
            raise HTTPException(
                status_code=400,
                detail="session_id and expected_session_fingerprint must be supplied together.",
            )
        ...
        if session is not None:
            ...
            try:
                ...
                raise HTTPException(
                    status_code=409,
                    detail=(
                        "Research result was persisted, but the session changed before the turn "
                        f"could be appended. Result ID: {stored.result_id}"
                    ),
                ) from exc
            except Exception as exc:
                raise HTTPException(
                    status_code=503,
                    detail=(
                        "Research result was persisted, but the session turn could not be appended. "
                        f"Result ID: {stored.result_id}"
                    ),
                ) from exc

        payload = dict(_result_payload(stored))
    ...
    async def get_research_result(
        ...
        owner = _owner(principal)
        try:
            return _result_payload(result_store.get(owner, result_id))
        except KeyError as exc:
