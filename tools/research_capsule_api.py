"""Owner-scoped research-capsule preflight, creation and inspection APIs.
...
def _replay_status(
    ...
    try:
        recipe = replay_recipe_store.metadata(owner, context.result.result_id)
    except KeyError:
    ...
    *,
    ...
    stale = (
        stale_reasons(
            invalidation_store,
            owner,
            DependencyRef("result", context.result.result_id),
            maximum=100,
        )
        if invalidation_store is not None
        else ()
    )
    replay_available, replay_reasons = _replay_status(owner, context, replay_recipe_store)
    ...
    *,
    ...
    async def preflight_capsule(
        ...
        return _preflight_payload(
            owner,
            context,
            replay_recipe_store=replay_recipe_store,
            invalidation_store=invalidation_store,
        )
    ...
    async def create_capsule(
        ...
        if not bool(preflight["manifest_ready"]):
            raise HTTPException(
                status_code=409,
                detail={"message": "Research capsule bindings are incomplete.", "preflight": preflight},
            )
        if request.require_replay_ready and not bool(preflight["replay_ready"]):
            raise HTTPException(
                status_code=409,
                detail={"message": "Exact replay material is not currently available.", "preflight": preflight},
            )
        try:
    ...
    async def list_capsules(
        ...
        owner = _owner(principal)
        try:
            values = capsule_store.list(owner, project_id=project_id, result_id=result_id, limit=limit)
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
        except Exception as exc:
            raise HTTPException(status_code=503, detail="Research capsule persistence is unavailable.") from exc
    ...
    async def get_capsule(
        ...
        owner = _owner(principal)
        try:
            stored = capsule_store.get(owner, capsule_id)
        except KeyError as exc:
