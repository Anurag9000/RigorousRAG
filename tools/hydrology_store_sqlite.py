        ...
        with self._lock, self._connect() as connection:
            ...
            try:
                current = connection.execute(
                    """SELECT version,fingerprint FROM hydrology_artifact_current
                       WHERE owner_id=? AND project_id=? AND kind=? AND logical_id=?""",
                ...
                if current is None:
                    ...
                    if current_fingerprint == envelope.fingerprint:
                        row = connection.execute(
                            """SELECT owner_id,project_id,kind,logical_id,version,fingerprint,created_at
                               FROM hydrology_artifact_versions
                               WHERE owner_id=? AND project_id=? AND kind=? AND logical_id=? AND version=?""",
                ...
                connection.execute(
                    """INSERT INTO hydrology_artifact_versions
                ...
                if current is None:
                    connection.execute(
                        """INSERT INTO hydrology_artifact_current
                    ...
                    cursor = connection.execute(
                        """UPDATE hydrology_artifact_current SET version=?,fingerprint=?,updated_at=?
                           WHERE owner_id=? AND project_id=? AND kind=? AND logical_id=? AND version=?""",
                        (
                            next_version,
                            envelope.fingerprint,
        ...
        *,
        ...
        with self._connect() as connection:
            if requested is None:
                row = connection.execute(
                    """SELECT v.fingerprint,v.schema_version,v.payload,v.created_at
                       FROM hydrology_artifact_current c
                       JOIN hydrology_artifact_versions v
                ...
                row = connection.execute(
                    """SELECT fingerprint,schema_version,payload,created_at
                       FROM hydrology_artifact_versions
                       WHERE owner_id=? AND project_id=? AND kind=? AND logical_id=? AND fingerprint=?
            ...
            if include_history:
                rows = connection.execute(
                    f"""SELECT v.owner_id,v.project_id,v.kind,v.logical_id,v.version,v.fingerprint,v.created_at,
                               CASE WHEN c.version=v.version THEN 1 ELSE 0 END AS is_current
                        FROM hydrology_artifact_versions v
                        LEFT JOIN hydrology_artifact_current c
                          ON c.owner_id=v.owner_id AND c.project_id=v.project_id AND c.kind=v.kind AND c.logical_id=v.logical_id
                        WHERE v.owner_id=? AND v.project_id=?{kind_clause}
                        ORDER BY v.created_at DESC,v.kind,v.logical_id,v.version DESC LIMIT ?""",
                    (*params, limit),
                ).fetchall()
            ...
            rows = connection.execute(
                f"""SELECT v.owner_id,v.project_id,v.kind,v.logical_id,v.version,v.fingerprint,v.created_at
                    ...
                    JOIN hydrology_artifact_versions v
                     ...
                     AND v.logical_id=c.logical_id AND v.version=c.version
                    WHERE v.owner_id=? AND v.project_id=?{kind_clause}
                    ORDER BY v.created_at DESC,v.kind,v.logical_id LIMIT ?""",
                (*params, limit),
            ).fetchall()
