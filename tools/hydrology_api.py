...
The API never performs local-file discovery or executes HEC models. It accepts already
materialized typed payloads, reconstructs and verifies them, binds engineering packages to
a persisted project topology, and creates retrieval plans server-side from the package's
authoritative spatiotemporal records.
"""
...
def _query_spec(value: Mapping[str, Any]):
    # Reuse the exact persistence codec without exposing a second interpretation of dates,
    # CRS identity or topology scope. A temporary plan-shaped object is unnecessary: the
    # codec's internal spec parser is intentionally imported here at the same module layer.
    from tools.hydrology_store import _spec_from_payload

    return _spec_from_payload(value)
    ...
    *,
    ...
    async def put_hydrology_topology(
        ...
        try:
            ...
            stored = store.put(envelope, expected_current_fingerprint=request.expected_current_fingerprint)
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
        except RuntimeError as exc:
            raise HTTPException(status_code=409, detail=str(exc)) from exc
    ...
    async def put_hydrology_package(
        ...
        actor = _owner(principal)
        access = _project_access(access_resolver, actor, project_id, "hydrology.write")
        topology_envelope = _get_artifact(
            store,
            access.storage_owner_id,
            access.project.project_id,
            "topology",
    ...
    async def create_hydrology_plan(
        ...
        package_envelope = _get_artifact(
            store,
            access.storage_owner_id,
            access.project.project_id,
            "engineering_package",
