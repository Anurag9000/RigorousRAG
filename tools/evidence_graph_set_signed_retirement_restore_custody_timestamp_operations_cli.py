"""Read-only custody timestamp-authority rotation audit CLI."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=(
            "python -m tools."
            "evidence_graph_set_signed_retirement_restore_custody_timestamp_operations_cli"
        ),
        description=(
            "Audit custody timestamp-authority keys against a bounded rotation policy. "
            "The command never registers, retires, signs, or deletes keys."
        ),
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        policy = CustodyTimestampRotationPolicy(
            maximum_active_key_age_seconds=args.maximum_active_key_age_seconds,
        ...
        report = assess_custody_timestamp_authority_rotation(
            ...
            registry=ReadOnlyCustodyTimestampAuthorityRegistry(path),
            policy=policy,
            limit=args.limit,
        )
        ...
        payload.update(
            {
                "policy": {
                    "maximum_active_key_age_seconds": (
                        policy.maximum_active_key_age_seconds
                    ),
