#!/usr/bin/env python3
"""Universal controller v28: execute source-proven declared combinations.

v28 preserves v27's exact byte-pinned OPF_ADP scheduler, lifecycle/DAG,
source-contract, exact-resume, early-stopping, registry/member/config/selector and
declared-combination closure layers.  It adds conservative execution
materialization for repository-authored combinations when one existing trainer
unambiguously exposes every required scientific selector.

No Cartesian products are invented.  Ambiguous or non-selectable declarations
remain fail-closed coverage blockers for repository-specific repair.

All CPU/GPU/resource admission and process scheduling remains exclusively in the
unchanged pinned OPF_ADP massive-suite scheduler.
"""
...
def main() -> int:
    # One shared vocabulary must exist before any inventory/materialization layer.
    ...
    # v28 closes concrete repo-authored combination gaps before trainer contracts
    # are wired, so every generated training job receives the same native
    # early-stopping/exact-resume treatment as curated jobs.
