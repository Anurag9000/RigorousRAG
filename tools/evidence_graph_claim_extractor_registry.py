...
from tools.evidence_graph_claim_extraction import ScientificClaimExtractionBatch
...
def _scope_values(
    ...
    *,
    ...
    if allowed is not None and (
        "*" in selected or any(item not in allowed for item in selected)
    ):
...
class ScientificClaimExtractorRecord:
    ...
    def __post_init__(self) -> None:
        ...
        output_schema = _digest(
            self.output_schema_sha256,
            "output_schema_sha256",
        )
        if output_schema != SCIENTIFIC_CLAIM_OUTPUT_SCHEMA_SHA256:
            raise ValueError(
                "extractor output schema differs from the supported claim schema."
            )
        ...
        if state == "active":
            if any(value is not None for value in retired_values):
                raise ValueError(
                    "active extractor may not contain retirement fields."
                )
            retired_actor = None
            retired_method = None
            ...
            if any(value is None for value in retired_values):
                raise ValueError(
                    "retired extractor requires complete retirement fields."
                )
            ...
            if retired_at < registered_at:
                raise ValueError(
                    "extractor retirement predates registration."
                )
        ...
        if record_digest != _canonical_digest(stable):
            raise ValueError(
                "record_digest differs from extractor record."
            )
        for key, value in {
            "owner_id": owner,
            "extractor_name": name,
    ...
    def active(
        ...
        *,
        ...
        values = {
            ...
            "extractor_name": _identifier(
                extractor_name,
                "extractor_name",
                200,
            ),
            "extractor_version": _identifier(
                extractor_version,
                "extractor_version",
                200,
            ),
            "extractor_kind": _identifier(
                extractor_kind,
                "extractor_kind",
                20,
            ),
            ...
            "supported_claim_types": _scope_values(
                tuple(supported_claim_types),
                "supported_claim_types",
                allowed=CLAIM_TYPES,
            ),
            "supported_modalities": _scope_values(
                tuple(supported_modalities),
                "supported_modalities",
                allowed=CLAIM_MODALITIES,
            ),
        ...
        *,
        ...
        if self.state == "retired":
            if (
                ...
                raise RuntimeError(
                    "extractor is already retired by another actor binding."
                )
            return self
        values = {
            **asdict(self),
...
class ClaimExtractorAdministratorGrant:
    ...
    def __post_init__(self) -> None:
        ...
        actions = _scope_values(
            self.actions,
            "actions",
        )
        if actions == ("*",) or any(
            value not in EXTRACTOR_ACTIONS
            for value in actions
        ):
            raise ValueError(
                "actions contains an unsupported value."
            )
        object.__setattr__(
            self,
            "actions",
        ...
        if self.schema_version != 1:
            raise ValueError(
                "extractor administrator grant schema is unsupported."
            )
...
class ClaimExtractorGovernancePolicy:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError(
                "administrators must be a bounded non-empty tuple."
            )
        ordered = tuple(
            sorted(
                self.administrators,
                key=lambda value: value.administrator_id,
            )
        )
        if len(
            {
            ...
            raise ValueError(
                "extractor administrator IDs must be unique."
            )
        object.__setattr__(
            self,
            "administrators",
        ...
        if self.schema_version != 1:
            raise ValueError(
                "extractor governance policy schema is unsupported."
            )
    ...
    def grant_for(
        ...
        raise PermissionError(
            "extractor administrator is not authorized."
        )
    ...
    def from_mapping(
        ...
        if not isinstance(value, Mapping) or set(value) != {
            ...
            raise ValueError(
                "extractor governance policy schema is invalid."
            )
        ...
        if (
            ...
            raise ValueError(
                "administrators must be a bounded non-empty array."
            )
        allowed = {
            "administrator_id",
            "owners",
        ...
        grants: list[ClaimExtractorAdministratorGrant] = []
        for raw in raw_values:
            if (
                ...
                raise ValueError(
                    "extractor administrator grant schema is invalid."
                )
            grants.append(
                ClaimExtractorAdministratorGrant(
                    administrator_id=raw[
                        "administrator_id"
                    ],
...
def load_claim_extractor_governance_policy(
    *,
    ...
    if (path is None) == (policy_json is None):
        raise RuntimeError(
            "configure exactly one claim extractor governance policy source."
        )
    if path is not None:
        selected = _path(
            path,
            "extractor governance policy path",
        )
        descriptor = os.open(
            selected,
            os.O_RDONLY
        ...
        try:
            ...
            if (
                ...
                raise ValueError(
                    "extractor governance policy file is invalid or too large."
                )
            payload = os.read(
                descriptor,
                _MAX_POLICY_BYTES + 1,
            ...
            if len(payload) > _MAX_POLICY_BYTES:
                raise ValueError(
                    "extractor governance policy file is too large."
                )
        finally:
            os.close(descriptor)
    else:
        if not isinstance(policy_json, str):
            raise ValueError(
                "extractor governance policy JSON must be text."
            )
        payload = policy_json.encode("utf-8")
        if not 1 <= len(payload) <= _MAX_POLICY_BYTES:
            raise ValueError(
                "extractor governance policy size is invalid."
            )
    try:
        raw = json.loads(
        ...
        raise ValueError(
            "extractor governance policy JSON is invalid."
        ) from exc
...
class ScientificClaimExtractorRegistry:
    def __init__(
        ...
        if (
            ...
            raise ValueError(
                "extractor registry parent must be a directory."
            )
    ...
    def _file_identity(self) -> tuple[int, int]:
        ...
        if (
            ...
            raise RuntimeError(
                "extractor registry is not a regular file."
            )
    ...
    def _verify(self) -> None:
        ...
        if (
            ...
            raise RuntimeError(
                "extractor registry identity changed."
            )
    ...
    def _value(
        ...
        try:
            ...
            raise RuntimeError(
                "stored extractor record is corrupt."
            ) from exc
        if (
            value.owner_id != row["owner_id"]
            or value.extractor_name
            != row["extractor_name"]
            or value.extractor_version
            != row["extractor_version"]
            or value.record_digest
            != row["record_digest"]
            or value.state != row["state"]
            or value.registered_at
            != float(row["registered_at"])
            or value.retired_at != row["retired_at"]
        ):
            raise RuntimeError(
                "stored extractor record columns are corrupt."
            )
        return value

    @staticmethod
    def _registration_scope(
        value: ScientificClaimExtractorRecord,
    ) -> tuple[Any, ...]:
        return (
            value.owner_id,
    ...
    def register(
        self,
        record: ScientificClaimExtractorRecord,
    ) -> ScientificClaimExtractorRecord:
        if (
            not isinstance(
                record,
            ...
            raise ValueError(
                "record must be an active ScientificClaimExtractorRecord."
            )
        payload = json.dumps(
            asdict(record),
            sort_keys=True,
        ...
        with self._lock, self._connect() as connection:
            ...
            try:
                ...
                if row is None:
                    connection.execute(
                        "INSERT INTO scientific_claim_extractors "
                        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)",
                        (
                            record.owner_id,
                            record.extractor_name,
                    ...
                    if self._registration_scope(
                        existing
                    ) != self._registration_scope(record):
                        raise RuntimeError(
                            "extractor version is already registered differently."
                        )
                    if existing.state == "retired":
                        raise RuntimeError(
                            "retired extractor versions may not be reactivated."
                        )
    ...
    def require_active(
        ...
        **kwargs: Any,
        ...
        if value.state != "active":
            raise PermissionError(
                "scientific claim extractor version is retired."
            )
        return value
            ...
            try:
                ...
                if observed.record_digest != current.record_digest:
                    if (
                        ...
                        return observed
                    raise RuntimeError(
                        "extractor record changed during retirement."
                    )
                cursor = connection.execute(
                    "UPDATE scientific_claim_extractors "
                    "SET payload_json=?, record_digest=?, "
                ...
                if cursor.rowcount != 1:
                    raise RuntimeError(
                        "extractor retirement compare-and-swap failed."
                    )
                connection.execute("COMMIT")
            except Exception:
                connection.execute("ROLLBACK")
    ...
    def list(
        ...
        *,
        ...
        if (
            ...
            and selected_state not in _STATES
        ):
            raise ValueError(
                "extractor state is unsupported."
            )
        count = _integer(
            limit,
            "limit",
        ...
        query += (
            " ORDER BY extractor_name, registered_at, "
...
class GovernedScientificClaimExtractorService:
    def __init__(
        ...
        *,
        ...
        if not isinstance(
            ...
            raise ValueError(
                "registry must be ScientificClaimExtractorRegistry."
            )
        if not isinstance(
            policy,
            ClaimExtractorGovernancePolicy,
            ...
            raise ValueError(
                "policy must be ClaimExtractorGovernancePolicy."
            )
    ...
    def _authorize(
        ...
        *,
        ...
        if not isinstance(actor, ReviewActorBinding):
            raise ValueError(
                "actor must be ReviewActorBinding."
            )
        require_relation_review_actor(
            actor.actor_id,
            binding=actor,
        ...
        if selected_action not in EXTRACTOR_ACTIONS:
            raise ValueError(
                "extractor action is unsupported."
            )
        now = _timestamp(
            self.clock(),
            "governance time",
        ...
        if not grant.permits(
            ...
            raise PermissionError(
                "extractor administrator grant does not permit this scope."
            )
        return now
    ...
    def register(
        ...
        **kwargs: Any,
        ...
        return self.registry.register(
            ScientificClaimExtractorRecord.active(
                actor=actor,
                now=now,
                **kwargs,
            )
...
def extract_registered_scientific_claim_proposals(
    ...
    *,
    ...
    """Compatibility alias for the canonical governed extraction boundary."""

    from tools.evidence_graph_claim_registered_extraction import (
        extract_governed_scientific_claim_proposals,
    )

    return extract_governed_scientific_claim_proposals(
        document,
        extractor_output,
        owner_id=owner_id,
