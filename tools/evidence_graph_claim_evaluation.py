...
def _f1(left: tuple[str, ...], right: tuple[str, ...]) -> float:
    ...
    overlap = sum(min(count, right_counts.get(key, 0)) for key, count in left_counts.items())
    if overlap == 0:
        return 0.0
...
def _span_iou(left: ClaimEvidenceLocator, right: ClaimEvidenceLocator) -> float:
...
class ScientificClaimGold:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "gold_id", _identifier(self.gold_id, "gold_id", 500))
...
class ScientificClaimEvaluationMatch:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "gold_id", _identifier(self.gold_id, "gold_id", 500))
        for name in ("gold_digest", "proposal_id", "proposal_digest"):
            object.__setattr__(self, name, _digest(getattr(self, name), name))
        for name in ("exact_evidence_digest", "exact_locator", "claim_type_correct", "modality_correct"):
            if not isinstance(getattr(self, name), bool):
                raise ValueError(f"{name} must be boolean.")
        object.__setattr__(self, "span_iou", _finite_metric(self.span_iou, "span_iou"))
        object.__setattr__(
            self,
            "claim_token_f1",
...
class ScientificClaimEvaluationReport:
    ...
    def __post_init__(self) -> None:
        ...
        for name in ("gold_count", "proposal_count", "matched_count"):
            object.__setattr__(self, name, _integer(getattr(self, name), name, 0, _MAX_CASES))
        if self.matched_count > min(self.gold_count, self.proposal_count):
            raise ValueError("matched_count exceeds available claims.")
        for name in (
            "precision",
            "recall",
            "f1",
            ...
            object.__setattr__(self, name, _finite_metric(getattr(self, name), name))
        if not isinstance(self.unmatched_gold_ids, tuple) or any(
            not isinstance(value, str) for value in self.unmatched_gold_ids
        ):
            raise ValueError("unmatched_gold_ids must be a tuple of IDs.")
        object.__setattr__(
            self,
            "unmatched_gold_ids",
            tuple(_identifier(value, "gold_id", 500) for value in self.unmatched_gold_ids),
        )
        if not isinstance(self.unmatched_proposal_ids, tuple):
            raise ValueError("unmatched_proposal_ids must be a tuple.")
        object.__setattr__(
            self,
            "unmatched_proposal_ids",
            tuple(_digest(value, "proposal_id") for value in self.unmatched_proposal_ids),
        )
        if not isinstance(self.matches, tuple) or any(
            not isinstance(value, ScientificClaimEvaluationMatch) for value in self.matches
        ):
            raise ValueError("matches must be a tuple of evaluation matches.")
        if len(self.matches) != self.matched_count:
            raise ValueError("match count differs from matches.")
        if len(self.unmatched_gold_ids) != self.gold_count - self.matched_count:
            raise ValueError("unmatched gold count is inconsistent.")
        if len(self.unmatched_proposal_ids) != self.proposal_count - self.matched_count:
            raise ValueError("unmatched proposal count is inconsistent.")
        for name in (
            "contains_claim_text",
            ...
            if getattr(self, name) is not False:
                raise ValueError("evaluation safety flags must remain false.")
        object.__setattr__(self, "report_digest", _digest(self.report_digest, "report_digest"))
        if self.schema_version != 1:
            raise ValueError("scientific claim evaluation schema is unsupported.")
...
def evaluate_scientific_claim_extraction(
    *,
    ...
    if not 1 <= len(gold_values) <= _MAX_CASES:
        raise ValueError("gold must contain a bounded non-empty set.")
    if len(proposal_values) > _MAX_CASES:
        raise ValueError("proposals exceeds the item limit.")
    if any(not isinstance(value, ScientificClaimGold) for value in gold_values):
        raise ValueError("every gold value must be ScientificClaimGold.")
    if any(not isinstance(value, ScientificClaimProposal) for value in proposal_values):
    ...
    if len({value.gold_id for value in gold_values}) != len(gold_values):
        raise ValueError("gold contains duplicate IDs.")
    if len({value.proposal_id for value in proposal_values}) != len(proposal_values):
    ...
    for value in (*gold_values, *proposal_values):
        ...
        if current != scope:
            raise PermissionError("claim evaluation values differ in generation scope.")

    candidates: list[tuple[tuple[Any, ...], ScientificClaimGold, ScientificClaimProposal, float, float]] = []
    for gold_value in gold_values:
        gold_tokens = _tokens(gold_value.claim_text)
        for proposal in proposal_values:
            span_iou = _span_iou(gold_value.locator, proposal.locator)
            text_f1 = _f1(gold_tokens, _tokens(proposal.claim_text))
            exact_evidence = (
                gold_value.locator.evidence_sha256
                == proposal.locator.evidence_sha256
            )
            eligible = exact_evidence or (
                span_iou >= span_threshold and text_f1 >= text_threshold
            )
            if not eligible:
                continue
            score = (
                ...
                span_iou,
                text_f1,
                int(gold_value.claim_type == proposal.claim_type),
                int(gold_value.modality == proposal.modality),
                gold_value.gold_id,
                proposal.proposal_id,
            )
            candidates.append((score, gold_value, proposal, span_iou, text_f1))

    used_gold: set[str] = set()
    used_proposals: set[str] = set()
    matches: list[ScientificClaimEvaluationMatch] = []
    for _score, gold_value, proposal, span_iou, text_f1 in sorted(
        candidates,
        key=lambda value: (
            -value[0][0],
            -value[0][1],
        ...
        if gold_value.gold_id in used_gold or proposal.proposal_id in used_proposals:
            continue
        used_gold.add(gold_value.gold_id)
        used_proposals.add(proposal.proposal_id)
        exact_locator = bool(
            gold_value.locator.section_index == proposal.locator.section_index
            and gold_value.locator.page_number == proposal.locator.page_number
            and gold_value.locator.char_start == proposal.locator.char_start
            and gold_value.locator.char_end == proposal.locator.char_end
        )
        matches.append(
            ScientificClaimEvaluationMatch(
                ...
                claim_type_correct=gold_value.claim_type == proposal.claim_type,
                modality_correct=gold_value.modality == proposal.modality,
            )
        )

    rendered_matches = tuple(sorted(matches, key=lambda value: (value.gold_id, value.proposal_id)))
    matched = len(rendered_matches)
    precision = _ratio(matched, len(proposal_values))
    recall = _ratio(matched, len(gold_values))
    unmatched_gold = tuple(
        sorted(value.gold_id for value in gold_values if value.gold_id not in used_gold)
    )
    ...
    exact_evidence_accuracy = _ratio(
        sum(value.exact_evidence_digest for value in rendered_matches), matched
    )
    exact_locator_accuracy = _ratio(
        sum(value.exact_locator for value in rendered_matches), matched
    )
    mean_span_iou = _ratio(
        sum(value.span_iou for value in rendered_matches), matched
    )
    mean_claim_token_f1 = _ratio(
        sum(value.claim_token_f1 for value in rendered_matches), matched
    )
    claim_type_accuracy = _ratio(
        sum(value.claim_type_correct for value in rendered_matches), matched
    )
    modality_accuracy = _ratio(
        sum(value.modality_correct for value in rendered_matches), matched
    )
    confidence_brier = _ratio(
        sum(
            (value.confidence - float(value.proposal_id in used_proposals)) ** 2
            for value in proposal_values
        ),
        len(proposal_values),
    )
    stable = {
        ...
        "matches": [asdict(value) for value in rendered_matches],
