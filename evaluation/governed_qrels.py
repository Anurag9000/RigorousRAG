...
judgments. The v2 receipt also binds JSON/JSONL query/document/relevance field mappings so exact
parsing semantics are reconstructible after process restart.
"""
...
class GovernedQrelsReceipt:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "source_path", str(safe_advanced_path(self.source_path, label="qrels receipt source", must_exist=True, require_file=True)))
        object.__setattr__(self, "source_sha256", _sha(self.source_sha256, "source_sha256"))
        object.__setattr__(self, "relevant_pair_sha256", _sha(self.relevant_pair_sha256, "relevant_pair_sha256"))
        object.__setattr__(self, "receipt_sha256", _sha(self.receipt_sha256, "receipt_sha256"))
        ...
        for name in ("query_field", "document_field", "relevance_field"):
            object.__setattr__(self, name, _identifier(getattr(self, name), name, 300))
        for name in ("pair_count", "query_count", "document_count"):
            value = getattr(self, name)
            if isinstance(value, bool) or not isinstance(value, int) or value < 0:
                raise ValueError(f"{name} must be non-negative")
        if _digest(self.unsigned()) != self.receipt_sha256:
            raise ValueError("governed qrels receipt digest mismatch")
    ...
    def unsigned(self) -> Mapping[str, Any]:
        return {
            ...
            "relevance_field": self.relevance_field,
            "pair_count": self.pair_count,
            "query_count": self.query_count,
            "document_count": self.document_count,
            "relevant_pair_sha256": self.relevant_pair_sha256,
        }
...
class GovernedQrels:
    ...
    def contract_sha256(self) -> str:
        return _digest({"schema": "rigorousrag-governed-qrels-contract/v2", "receipt_sha256": self.receipt.receipt_sha256})
...
def _entry_from_mapping(value: Any, label: str, *, query_field: str, document_field: str, relevance_field: str) -> QrelsEntry:
    ...
    for field in (query_field, document_field, relevance_field):
        if field not in value:
            raise ValueError(f"{label} lacks field {field!r}")
...
def _entries(path: Path, *, input_format: str, query_field: str, document_field: str, relevance_field: str) -> Iterator[QrelsEntry]:
        ...
        with path.open("r", encoding="utf-8", errors="strict") as handle:
            for line_number, line in enumerate(handle, start=1):
                ...
                if len(line.encode("utf-8")) > _MAX_LINE_BYTES:
                    raise ValueError(f"qrels line {line_number} exceeds safety bound")
                fields = line.strip().split()
                if len(fields) == 4:
                    query_id, _, document_id, relevance = fields
                elif len(fields) == 3:
                    query_id, document_id, relevance = fields
                else:
                    raise ValueError(f"qrels TREC line {line_number} must have 3 or 4 whitespace-separated fields")
    ...
    if path.stat().st_size <= 0 or path.stat().st_size > _MAX_BYTES:
        raise ValueError("qrels JSON exceeds whole-file safety bound")
    value = _strict_json(path.read_bytes(), "qrels JSON")
    if not isinstance(value, list):
        raise ValueError("qrels JSON must contain an array")
    for index, item in enumerate(value):
        yield _entry_from_mapping(item, f"qrels[{index}]", query_field=query_field, document_field=document_field, relevance_field=relevance_field)
...
def load_governed_qrels(
    ...
    *,
    ...
    count = 0
    for entry in _entries(source, input_format=selected_format, query_field=selected_query_field, document_field=selected_document_field, relevance_field=selected_relevance_field):
        count += 1
        ...
        if pair in pairs:
            continue
        pairs.add(pair)
        relevant.setdefault(entry.query_id, set()).add(entry.document_id)
        documents.add(entry.document_id)
    ordered_pairs = sorted(f"{query}\t{document}" for query, document in pairs)
    ...
    receipt = GovernedQrelsReceipt(
        str(source), source_sha, selected_format, threshold,
        selected_query_field, selected_document_field, selected_relevance_field,
        len(pairs), len(relevant), len(documents), pair_sha, _digest(unsigned),
    )
...
def overlay_qrels(
    ...
    qrels: GovernedQrels,
    *,
    require_query_labels: bool = True,
    require_existing_equal: bool = True,
    ...
    for example in examples:
        if not isinstance(example, BenchmarkExample):
            raise ValueError("examples must contain BenchmarkExample values")
        gold = qrels.relevant_ids(example.example_id)
        existing = tuple(example.relevant_ids)
        if require_query_labels and not gold:
            raise ValueError(f"qrels contain no relevant documents for query {example.example_id!r}")
        if existing and gold and require_existing_equal and set(existing) != set(gold):
            raise ValueError(f"embedded relevant ids conflict with governed qrels for query {example.example_id!r}")
        selected = gold or existing
        metadata = dict(example.metadata)
        metadata["qrels_receipt_sha256"] = qrels.receipt.receipt_sha256
