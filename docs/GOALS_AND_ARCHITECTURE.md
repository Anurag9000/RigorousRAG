...
- the built-in single-host scheduler/executors are a distributed exactly-once system.
...
- Deduplicated daemon timers release retries without occupying workers.
...
- One process-wide bounded tool executor instead of a pool per request.
...
- Strict reflected request IDs and bounded model/job/document identifiers.
...
graph TD
    ...
    Queue --> Scheduler[Persisted deadlines + daemon timers]
    Scheduler --> Workers[Bounded workers / atomic due claims]
    ...
    Agent --> ToolPool[Process-wide bounded tool executor]
...
- Browser input and API path/header values are untrusted.
...
1. The service receives a bounded supported file under a random owner-scoped storage name and `fsync`s it.
2. A durable owner-scoped `queued` job records its private source path.
3. The scheduler releases the job when its persisted deadline is due.
4. One worker atomically claims it as `processing`.
5. The parser validates signatures, archive/PDF/text complexity, and extracts native/OCR text.
6. Every text and metadata representation is masked.
7. A stable owner-and-source document ID is computed without exporting the source hash.
8. The source identity is reverified immediately before summary/vector writes.
9. Redacted semantic sections are indexed with deterministic child IDs; failed replacement invokes compensating restoration.
10. The job enters `finalizing`, then the private registry records the retained source or text-only state.
11. The job becomes `success` only after vector and registry handling finish.
12. Transient failures return to `queued` with a persisted exponential deadline; exhausted failures clear unusable document IDs.
13. Retrieval always includes the authenticated owner's filter.
14. Document deletion removes vector chunks, registry state, and any retained source.
15. Startup reconciliation reschedules interrupted jobs or promotes durable finalization state.
16. Orphan reconciliation deletes only old regular files unreferenced by active jobs or retained documents.
...
- contract and regression tests for identity, connected-peer SSRF, redirect secrets, total network deadlines, upload durability, archive/parser bounds, masking, OCR, source revalidation, durable claims/retry scheduling/recovery, registry isolation, vector rollback/pagination/outages, provenance, tool timeout/schema/concurrency, scientific fail-closed behavior, ranking, storage, telemetry rotation, readiness, and frontend safety;
