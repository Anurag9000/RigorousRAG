...
1. ``plan_dynamic_hidden_supervision`` attaches reviewed need spans and deterministic cache
   keys but writes no hidden tensors;
...
3. ``materialize_manifest_bound_hidden_cache`` executes the admitted generator later and
   writes the exact keys into a cache whose identity is bound to the final manifest SHA.
...
def _normalized_hidden(encoded: Mapping[str, Any]) -> Mapping[str, Any]:
    if torch is None: raise RuntimeError("manifest-bound hidden cache materialization requires optional PyTorch")
    required = {"token_hidden", "state_hidden", "attention_mask"}
    if not isinstance(encoded, Mapping) or not required.issubset(encoded): raise ValueError("hidden provider must return token_hidden/state_hidden/attention_mask")
    token_hidden, state_hidden, attention = encoded["token_hidden"], encoded["state_hidden"], encoded["attention_mask"]
    if not all(torch.is_tensor(value) for value in (token_hidden, state_hidden, attention)): raise ValueError("hidden provider outputs must be tensors")
    token_hidden, state_hidden, attention = token_hidden.detach().cpu(), state_hidden.detach().cpu(), attention.detach().cpu()
    if token_hidden.ndim != 3 or token_hidden.size(0) != 1 or state_hidden.ndim != 2 or state_hidden.size(0) != 1 or attention.ndim != 2 or attention.size(0) != 1: raise ValueError("hidden provider must return batch-one [1,T,H]/[1,H]/[1,T]")
    if token_hidden.size(1) != attention.size(1) or token_hidden.size(2) != state_hidden.size(1): raise ValueError("hidden provider tensor shapes are inconsistent")
    if not bool(attention[0].to(dtype=torch.bool).any().item()): raise ValueError("hidden provider returned no visible token")
...
class DynamicHiddenSupervisionPlanReceipt:
    ...
    def __post_init__(self) -> None:
        for name in ("hidden_provider_sha256", "keyset_sha256", "planned_records_sha256", "receipt_sha256"): object.__setattr__(self, name, _sha(getattr(self, name), name))
        if self.annotation_provider_sha256 is not None: object.__setattr__(self, "annotation_provider_sha256", _sha(self.annotation_provider_sha256, "annotation_provider_sha256"))
        if isinstance(self.record_count, bool) or not isinstance(self.record_count, int) or self.record_count <= 0: raise ValueError("record_count must be positive")
        if _digest(self.unsigned()) != self.receipt_sha256: raise ValueError("dynamic hidden-supervision plan receipt digest mismatch")
...
class ManifestBoundHiddenCacheReceipt:
    ...
    def __post_init__(self) -> None:
        for name in ("dataset_manifest_sha256", "cache_identity_sha256", "cache_contract_sha256", "hidden_provider_sha256", "keyset_sha256", "entryset_sha256", "receipt_sha256"): object.__setattr__(self, name, _sha(getattr(self, name), name))
        if isinstance(self.record_count, bool) or not isinstance(self.record_count, int) or self.record_count <= 0: raise ValueError("record_count must be positive")
        if _digest(self.unsigned()) != self.receipt_sha256: raise ValueError("manifest-bound hidden-cache receipt digest mismatch")
...
def plan_dynamic_hidden_supervision(
    ...
    if not selected or any(not isinstance(step, LegalDynamicRagEpisodeStep) for step in selected): raise ValueError("steps must contain LegalDynamicRagEpisodeStep values")
    ...
    if annotation_provider is None and require_need_annotations: raise ValueError("need-selection planning requires an explicit annotation provider")
    planned = []; keys = []; seen: set[str] = set()
    for step in selected:
        key = f"dynamic-hidden:{step.episode_id}:{step.step_id}"
        if key in seen: raise ValueError(f"duplicate hidden-state cache key {key}")
        seen.add(key); spans = tuple(annotation_provider.spans(step)) if annotation_provider is not None else tuple(step.need_spans)
        if any(not isinstance(span, TextSpan) or span.end > len(step.context) for span in spans): raise ValueError(f"invalid information-need spans for {step.episode_id}:{step.step_id}")
        metadata = dict(step.metadata); metadata["hidden_provider_sha256"] = provider_sha
        if annotation_sha is not None: metadata["need_annotation_provider_sha256"] = annotation_sha
        planned.append(replace(step, hidden_state_cache_key=key, need_spans=spans, metadata=metadata)); keys.append(key)
...
def materialize_manifest_bound_hidden_cache(
    ...
    if not isinstance(cache, AuthoritativeSafetensorSupervisionCache): raise ValueError("cache must be AuthoritativeSafetensorSupervisionCache")
    provider_sha = _sha(getattr(hidden_provider, "contract_sha256", None), "hidden provider contract_sha256")
    ...
    if identity.cache_kind != "generator_hidden_states": raise ValueError("cache_kind must be generator_hidden_states")
    if identity.dataset_manifest_sha256 != dataset.manifest.manifest_digest: raise ValueError("hidden cache identity must bind the final dynamic dataset manifest")
    if identity.producer_sha256 != generator_sha or identity.tokenizer_sha256 != tokenizer_sha: raise ValueError("hidden cache producer/tokenizer identity differs from provider")
    keys: list[str] = []; entries: list[str] = []; count = 0; seen: set[str] = set()
    for split in dataset.manifest.splits:
        records = dataset.split(split.name)
        for index in range(len(records)):
            step = records[index]; key = step.hidden_state_cache_key
            if key is None: raise ValueError(f"published step {step.episode_id}:{step.step_id} lacks hidden_state_cache_key")
            if key != f"dynamic-hidden:{step.episode_id}:{step.step_id}": raise ValueError("published hidden-state key differs from canonical deterministic key")
            if key in seen: raise ValueError(f"duplicate hidden-state cache key across final splits: {key}")
            if step.metadata.get("hidden_provider_sha256") != provider_sha: raise ValueError("published step hidden-provider identity differs from materializer")
            seen.add(key); tensors = _normalized_hidden(hidden_provider.encode([step.context])); entries.append(cache.put(key, tensors)); keys.append(key); count += 1
    contract = cache.contract_sha256  # seals and verifies the complete final cache
