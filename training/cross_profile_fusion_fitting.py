...
The learner never consumes heterogeneous raw retrieval scores. Each feature is a held-out
calibrated relevance probability from a governed retriever profile. Softmax parameters
produce non-negative weights that sum to one; the weighted profile logits are mapped back
to a fused probability and optimized with weighted binary log-loss.

Training is exactly resumable at mini-batch boundaries. Epoch permutations derive from
immutable seed+epoch identity, so resumption does not depend on hidden RNG state. Final
artifacts bind calibration contract, calibrator artifacts, train/validation data,
...
def _git_revision(value: Any) -> str:
    ...
    if len(selected) not in (40, 64) or any(
        ch not in "0123456789abcdef" for ch in selected
    ):
        raise ValueError(
            "source_revision must be a 40- or 64-character hexadecimal Git object id."
        )
    return selected
...
def _positive(value: Any, label: str, *, allow_zero: bool = False) -> float:
    ...
    if (
        ...
        or selected < 0.0
        or (not allow_zero and selected == 0.0)
    ):
        raise ValueError(f"{label} is invalid.")
    return selected
...
class FusionWeightExample:
    ...
    def __post_init__(self) -> None:
        ...
        for key, value in self.probabilities.items():
            profile = _identifier(key, "profile id")
            if profile in cleaned:
                raise ValueError("profile probabilities must be unique.")
            cleaned[profile] = _probability(value, "profile probability")
...
class FusionWeightTrainingConfig:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            ...
            _positive(self.positive_class_weight, "positive_class_weight"),
        )
        object.__setattr__(
            self,
            "min_delta",
            _positive(self.min_delta, "min_delta", allow_zero=True),
        )
    ...
    def config_sha256(self) -> str:
        return _canonical_digest(
            {
                "schema": "rigorousrag-fusion-weight-training-config/v1",
                **asdict(self),
            }
        )
...
class FusionWeightTrainingSpec:
    ...
    def __post_init__(self) -> None:
        profiles = tuple(
            sorted(_identifier(value, "profile id") for value in self.profile_ids)
        )
        ...
        object.__setattr__(
            self,
            "train_split_sha256",
            _sha256(self.train_split_sha256, "train_split_sha256"),
        )
        object.__setattr__(
            self,
            "validation_split_sha256",
            _sha256(self.validation_split_sha256, "validation_split_sha256"),
        )
        ...
        artifacts = tuple(
            sorted(
                (
                    _identifier(profile, "artifact profile id"),
                    _sha256(digest, "calibration artifact sha256"),
                )
                for profile, digest in self.calibration_artifact_sha256s
            )
        )
...
def examples_sha256(
    examples: Sequence[FusionWeightExample], profile_ids: Sequence[str]
) -> str:
    profiles = tuple(profile_ids)
    rows = []
    for item in examples:
        if not isinstance(item, FusionWeightExample):
            raise ValueError("examples must contain FusionWeightExample values.")
        if set(item.probabilities) != set(profiles):
            raise ValueError(
                "every fusion-weight example must exactly cover the training profiles."
            )
        rows.append(
            {
                "probabilities": tuple(
                    (profile, item.probabilities[profile]) for profile in profiles
                ),
    ...
    return _canonical_digest(
        {
            "schema": "rigorousrag-fusion-weight-examples/v1",
            "profiles": profiles,
            "examples": rows,
        }
    )
...
class FusionWeightTrainingState:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            not theta
            or len(theta) != len(best_theta)
            or any(not math.isfinite(value) for value in theta + best_theta)
        ):
        ...
        if self.best_validation_loss is not None:
            ...
            if not math.isfinite(value) or value < 0.0:
                raise ValueError(
                    "best_validation_loss must be finite and non-negative."
                )
            object.__setattr__(self, "best_validation_loss", value)
        if self.best_epoch is not None and (
            isinstance(self.best_epoch, bool)
            or not isinstance(self.best_epoch, int)
    ...
    def state_sha256(self) -> str:
        return _canonical_digest(
            {"schema": "rigorousrag-fusion-weight-training-state/v1", **asdict(self)}
        )
...
class LearnedFusionWeightArtifact:
    ...
    def __post_init__(self) -> None:
        ...
        weights = tuple(
            sorted(
                (_identifier(profile, "profile id"), float(weight))
                for profile, weight in self.profile_weights
            )
        )
        if not weights or any(
            not math.isfinite(weight) or weight < 0.0 for _, weight in weights
        ):
        ...
        if tuple(profile for profile, _ in artifacts) != tuple(
            profile for profile, _ in weights
        ):
            raise ValueError(
                "calibration artifacts must cover the learned weight profiles."
            )
        object.__setattr__(self, "calibration_artifact_sha256s", artifacts)
        if (
            isinstance(self.best_epoch, bool)
        ...
        if expected != provided:
            raise ValueError(
                "artifact_sha256 does not match learned fusion artifact content."
            )
    ...
    def build(
        ...
        *,
        ...
        state: FusionWeightTrainingState,
    ...
    def probability(self, probabilities: Mapping[str, float]) -> float:
        ...
        fused_logit = sum(
            weight * _logit(_probability(probabilities[profile], profile))
            for profile, weight in self.profile_weights
        )
...
def initialize_training_state(
    spec: FusionWeightTrainingSpec,
    train_examples: Sequence[FusionWeightExample],
    validation_examples: Sequence[FusionWeightExample],
...
def _epoch_order(count: int, *, seed: int, epoch: int) -> list[int]:
    mixed = int(
        hashlib.sha256(f"{seed}:{epoch}".encode("utf-8")).hexdigest()[:16], 16
    )
...
def _validation_loss(
    ...
    examples: Sequence[FusionWeightExample],
    ...
    for item in examples:
        ...
        z = sum(
            fusion_weights[index] * _logit(item.probabilities[profile])
            for index, profile in enumerate(profiles)
        )
        total_loss += effective * _example_loss(_sigmoid(z), item.relevant)
        total_weight += effective
    if total_weight <= 0.0:
        raise ValueError("validation examples have no effective weight.")
...
def advance_training(
    ...
    *,
    ...
    if (
        examples_sha256(train_examples, spec.profile_ids)
        != state.train_examples_sha256
        or examples_sha256(validation_examples, spec.profile_ids)
        != state.validation_examples_sha256
    ):
        raise ValueError(
            "training or validation examples differ from resumable state identity."
        )
    if len(state.theta) != len(spec.profile_ids):
        raise ValueError("training state parameter dimension differs from profile count.")
    if state.completed:
        return state
    ...
    while epoch < config.epochs:
        ...
        if batch_index > batch_count:
            raise ValueError("training state's batch_index exceeds the epoch batch count.")
        while batch_index < batch_count:
            start = batch_index * config.batch_size
            indices = order[start : start + config.batch_size]
            weights = _softmax(theta)

            # Normalize only the empirical data gradient by effective sample weight.
            # L2 is an objective term, not another sample contribution; adding it after
            # normalization makes its strength invariant to batch/sample weighting.
            data_gradient = [0.0 for _ in theta]
            normalizer = 0.0
            for row_index in indices:
                item = train_examples[row_index]
                effective = item.weight * (
                    config.positive_class_weight if item.relevant else 1.0
                )
                features = [
                    _logit(item.probabilities[profile]) for profile in spec.profile_ids
                ]
                z = sum(
                ...
                for index in range(len(data_gradient)):
                    data_gradient[index] += (
                        effective
                        * error
                        * weights[index]
            ...
            if normalizer <= 0.0:
                raise ValueError("training batch has no effective sample weight.")
            gradient = [
                data_gradient[index] / normalizer + config.l2 * theta[index]
            ...
            theta = [
                theta[index] - config.learning_rate * gradient[index]
                for index in range(len(theta))
            ]
            # Softmax is invariant to a common theta offset. Re-centering prevents
            # unnecessary drift in the non-identifiable all-ones direction.
