"""Strict config-only CLI for authoritative governed grounded training-data publication."""
...
from training.authoritative_governed_grounded_import import import_authoritative_governed_grounded_dataset
...
def _card(raw: Any) -> DatasetCard:
    ...
    if set(raw) - allowed:
        raise ValueError(
            f"governance.card has unsupported fields: {sorted(set(raw)-allowed)}"
        )
    return DatasetCard(
        summary=raw.get("summary", ""),
        intended_uses=_strings(raw.get("intended_uses", []), "card.intended_uses"),
        forbidden_uses=_strings(raw.get("forbidden_uses", []), "card.forbidden_uses"),
        populations_or_domains=_strings(
            raw.get("populations_or_domains", []),
            "card.populations_or_domains",
        ),
        languages=_strings(raw.get("languages", []), "card.languages"),
...
def _profile(name: str, raw: Any) -> DeclarativeGroundedProfile:
    ...
    tuple_fields = {
        ...
        "claim_end_paths",
        "claim_text_paths",
        "claim_evidence_id_paths",
        "claim_supporting_id_paths",
        "claim_contradicting_id_paths",
        "claim_supported_paths",
        "claim_contradicted_paths",
        "unsupported_start_paths",
        "unsupported_end_paths",
        "unsupported_text_paths",
        "abstain_paths",
        "reflection_action_paths",
        "chosen_answer_paths",
        "rejected_answer_paths",
    }
    kwargs: dict[str, Any] = {"name": name}
    for field_name in tuple_fields:
        kwargs[field_name] = _strings(
            raw.get(field_name, []),
            f"profile {name}.{field_name}",
        )
    ...
    for field_name in ("constant_abstain", "generate_missing_ids"):
        if field_name in raw:
            ...
            kwargs[field_name] = raw[field_name]
    if (
        "constant_reflection_action" in raw
        and raw["constant_reflection_action"] is not None
    ):
        kwargs["constant_reflection_action"] = ReflectionAction(
            raw["constant_reflection_action"]
        )
    ...
    kwargs["metadata_paths"] = {
        str(key): _strings(value, f"profile {name}.metadata_paths.{key}")
    ...
    kwargs["constant_metadata"] = {
        str(key): str(value) for key, value in constants.items()
    }
...
def run_import_config(path: str | Path) -> Mapping[str, Any]:
    raw = _read(path)
    if (
        set(raw) != {"schema", "output_dir", "governance", "profiles", "splits"}
        or raw.get("schema") != "rigorousrag-governed-grounded-import-config/v1"
    ):
        raise ValueError(
            "config must be rigorousrag-governed-grounded-import-config/v1"
        )
    ...
    tasks_raw = governance_raw.get("tasks")
    modalities_raw = governance_raw.get("modalities")
    if (
        not isinstance(tasks_raw, list)
        or not tasks_raw
        or not isinstance(modalities_raw, list)
    ...
    governance = GroundedDatasetGovernanceSpec(
        ...
        tasks=tuple(DatasetTask(item) for item in tasks_raw),
        modalities=tuple(DatasetModality(item) for item in modalities_raw),
        card=_card(governance_raw.get("card")),
    ...
    profiles = {
        str(name): _profile(str(name), value) for name, value in profiles_raw.items()
    }
    ...
    for index, item in enumerate(splits_raw):
        ...
        if profile_name not in profiles:
            raise ValueError(
                f"split {index} references unknown profile {profile_name!r}"
            )
        splits.append(
            GroundedSplitImportSpec(
                name=item.get("name"),
    ...
    manifest, receipt = import_authoritative_governed_grounded_dataset(
        governance,
        tuple(splits),
        output_dir=raw["output_dir"],
    )
    receipt_path = Path(raw["output_dir"]) / "import_receipt.json"
    verified = verify_governed_grounded_import(
        receipt_path,
    ...
    if (
        ...
        raise RuntimeError(
            "grounded import read-side verification returned a different identity"
        )
    return {
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Convert exact local annotated data into path-safe governed "
            "grounded-generator training JSONL"
        )
    )
    parser.add_argument(
        "config",
        help="rigorousrag-governed-grounded-import-config/v1 JSON file",
    )
