...
def test_timed_out_query_retains_capacity_until_worker_finishes(
    ...
    assert timed_out.json()["detail"] == "The research query exceeded the server time limit."
    assert saturated.status_code == 503
    assert saturated.headers["Retry-After"] == "1"
    assert saturated.json()["detail"] == "The research query executor is at capacity."


def test_query_executor_failure_is_not_exposed(server_module, monkeypatch):
    ...
    assert response.json()["detail"] == "The research query executor is at capacity."
