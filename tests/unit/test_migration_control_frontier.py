...
def test_sqlite_versioned_state_compare_and_set_and_owner_isolation(tmp_path):
    ...
    first = store.compare_and_set(owner_id="owner-a", key="policy", expected_version=None, value={"v": 1})
    assert first is not None and first.version == 1
    assert store.compare_and_set(owner_id="owner-a", key="policy", expected_version=None, value={"v": 2}) is None
    second = store.compare_and_set(owner_id="owner-a", key="policy", expected_version=1, value={"v": 2})
...
def test_migration_coordinator_commits_idempotently(tmp_path):
    ...
    created = coordinator.begin(owner_id="owner-a", transaction_id="tx-1", resource_id="corpus",
                                participants=(first, second), payload={"generation": 7})
    assert created.state == "created"
    committed = coordinator.execute(owner_id="owner-a", transaction_id="tx-1", participants=(first, second),
                                    coordinator_id="worker-1")
    assert committed.state == "committed" and committed.fencing_token >= 1
    replay = coordinator.execute(owner_id="owner-a", transaction_id="tx-1", participants=(first, second),
...
def test_migration_coordinator_compensates_commit_failure(tmp_path):
    first, second = FakeParticipant("first"), FakeParticipant("second", fail_commit=True)
    coordinator = MigrationCoordinator(tmp_path / "migration.sqlite3")
    coordinator.begin(owner_id="owner-a", transaction_id="tx-2", resource_id="index",
                      participants=(first, second), payload={"from": 1, "to": 2})
    result = coordinator.execute(owner_id="owner-a", transaction_id="tx-2", participants=(first, second),
                                 coordinator_id="worker-1")
...
def test_control_router_claim_resolve_feedback_and_owner_scope(tmp_path):
    ...
    reviews.enqueue(owner_id="owner-a", request_id="req-1",
                    decision=ReviewDecision("human_review", 0.8, ("high_uncertainty",)), query="secret query")
    reviews.enqueue(owner_id="owner-b", request_id="req-2",
                    decision=ReviewDecision("human_review", 0.9, ("evidence_conflict",)), query="other query")

    async def principal() -> Principal:
        return Principal(owner_id="owner-a", authenticated=True)

    app = FastAPI()
    app.include_router(build_control_router(principal_dependency=principal, review_store=reviews, feedback_store=feedback))
    client = TestClient(app)

    listed = client.get("/reviews").json()
    assert [item["request_id"] for item in listed] == ["req-1"]
    claimed = client.post("/reviews/claim", json={"reviewer_id": "reviewer-1", "ttl_seconds": 60}).json()
    assert claimed["request_id"] == "req-1" and claimed["query_sha256"] and "secret query" not in str(claimed)
    resolved = client.post("/reviews/req-1/resolve", json={
        "reviewer_id": "reviewer-1", "lease_token": claimed["lease_token"], "resolution": "approved"
    })
    assert resolved.status_code == 200 and resolved.json() == {"resolved": True}

    event = client.post("/feedback", json={
        "event_id": "fb-1", "kind": "answer_correct", "subject_id": "req-1", "query": "secret query"
    })
