...
from tools import migration_rollback_reconstruction_cli as cli
...
def test_typed_verify_is_path_free_and_nonmutating(tmp_path, monkeypatch, capsys):
    preflight = install(monkeypatch, tmp_path)
    assert cli.main([preflight.task_id]) == 0
    output, error = parse(capsys)
    assert error is None
    assert output["typed_reconstruction_verified"] is True
    assert output["vector_rows"] == 1
    assert output["sparse_fields"] == 1
...
def test_missing_key_and_missing_artifact_are_bounded(tmp_path, monkeypatch, capsys):
    preflight = install(monkeypatch, tmp_path)
