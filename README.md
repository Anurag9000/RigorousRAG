...
- CLI search interface (`Searching.py`) and AI-augmented assistant (`ai_search.py`) that produces citation-linked summaries via OpenAI or an extractive fallback.
...
> **Note**: Network access is required for crawling and for LLM calls. The OpenAI integration expects `OPENAI_API_KEY` in your environment unless you pass `--api-key` to `ai_search.py`.
...
python ai_search.py --query "climate change impact on coral reefs" --max-pages 180 --results 8
```

Without `--query`, the command enters an interactive loop (`Ask>` prompt). The output includes:
1. An LLM-generated summary with bracketed citations `[n]`.
2. A source list matching the citation numbers.
3. The underlying ranked documents.

If the OpenAI client cannot be initialised (no key, network, or library), the tool falls back to an extractive summary that still references the relevant documents.
