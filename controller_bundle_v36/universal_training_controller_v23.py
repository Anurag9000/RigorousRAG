#!/usr/bin/env python3
"""Universal controller v23: member-complete workload closure on literal OPF_ADP.

v23 is v22 plus fail-closed registry-member accounting.  It distinguishes
"the registry source is reachable" from "every implemented member in that
registry is actually represented by the central experiment universe".  This
closes omissions such as a BACKBONES dictionary containing three implemented
architectures while every job selects only one.

The literal byte-pinned OPF_ADP scheduler remains the sole resource/admission
engine.  No pressure, RAM/VRAM, concurrency, pause/resume, retry, OOM fallback,
device or process scheduling algorithm is implemented here.
"""
