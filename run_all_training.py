...
closed-world scientific catalog and invokes the current universal v28 bootstrap;
...
enabled with production paths/digests. v28 makes strong scientific registries,
CLI/Enum/Literal/registration selectors and component config groups for models,
losses/objectives, optimizers/schedulers, ensembles, workflows, metrics and
related surfaces fail closed when any declared member is not centrally reachable.
It also requires every repository-authored compatible scientific combination to
be represented together and materializes a missing combination automatically
when one existing trainer exposes every required selector unambiguously; it never
...
def main() -> int:
    if not CONTROLLER.is_file():
        raise RuntimeError(f"Universal v28 controller bootstrap is missing: {CONTROLLER}")
